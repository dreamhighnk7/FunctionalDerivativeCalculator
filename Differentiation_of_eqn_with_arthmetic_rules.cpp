...
{
	return "{(" + s2 + ")*(" + differentiation(s1,check_function(s1)) + ") + (" + s1 + ")*(" + differentiation(s2,check_function(s2)) + ")}" ;
}
...
string diff_expression(string func[], string oper[], int count)//Input : array of functions, operators, number of operators ;  Output:  String of the differentiated expression
{
	...
	for(int i=0 ; i<count ;i++)
	{
		...
		if(oper[i] == "+")    //If it is a sum we just differentiate it
			f_prime = differentiation(func[i],check_function(func[i])) ;
	...
	{
		temp = differentiation(func[k+1],check_function(func[k+1])) ;
		if(temp[0] == '-')
		{	
	...
	if(count == 0)
	{
		exp_prime += differentiation(func[0],check_function(func[0]));
		// cout << "f = " << func[0] << endl;
	}
